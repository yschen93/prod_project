...
RestServer::RestServer(AppConfig cfg, std::shared_ptr<spdlog::logger> logger)
...
{
  logger_->info("starting server on {}:{}", cfg_.bind_host, cfg_.port);
...
void RestServer::RegisterRoutes()
{
  ...
  server_.Get("/time", [this](const httplib::Request&, httplib::Response& res) {
    const absl::Time now = absl::Now();
    const std::string ts = absl::FormatTime("%Y-%m-%dT%H:%M:%E*S%Ez", now, absl::LocalTimeZone());
    nlohmann::json j;
    j["time"] = ts;
  ...
  server_.Post("/echo", [this](const httplib::Request& req, httplib::Response& res) {
    logger_->info("/echo content_type={} body_size={}", req.get_header_value("Content-Type"), req.body.size());
    ...
    if (in.contains("text") && in["text"].is_string())
    {
      ...
      out["formatted"] = absl::StrFormat("%s | len=%d", text, static_cast<int>(text.size()));
    }
