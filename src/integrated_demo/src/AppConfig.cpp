...
static std::optional<AppConfig> LoadConfigFromYamlNode(const YAML::Node& root)
{
  if (!root || !root.IsMap())
  {
    return std::nullopt;
  ...
  if (root["log_level"])
  {
    cfg.log_level = absl::AsciiStrToLower(root["log_level"].as<std::string>());
    cfg.log_level = std::string(absl::StripAsciiWhitespace(cfg.log_level));
  }
  if (root["greeting"])
  {
  ...
  {
    std::vector<std::string> parts = absl::StrSplit(cfg.greeting, ' ', absl::SkipWhitespace());
    if (!parts.empty())
    {
...
void ApplyJsonOverrides(AppConfig& cfg, const nlohmann::json& overrides)
{
  ...
  if (overrides.contains("bind_host") && overrides["bind_host"].is_string())
  {
    cfg.bind_host = overrides["bind_host"].get<std::string>();
  }
  if (overrides.contains("port") && overrides["port"].is_number_integer())
  {
    cfg.port = overrides["port"].get<int>();
  }
  if (overrides.contains("worker_threads") && overrides["worker_threads"].is_number_integer())
  {
    cfg.worker_threads = overrides["worker_threads"].get<int>();
  }
  if (overrides.contains("log_level") && overrides["log_level"].is_string())
  {
    cfg.log_level = absl::AsciiStrToLower(overrides["log_level"].get<std::string>());
    cfg.log_level = std::string(absl::StripAsciiWhitespace(cfg.log_level));
  }
  if (overrides.contains("greeting") && overrides["greeting"].is_string())
  {
